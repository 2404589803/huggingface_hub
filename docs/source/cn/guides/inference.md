...
推理是使用训练好的模型对新数据进行预测的过程。由于这个过程可能会消耗大量计算资源，因此在专用服务器上运行可能是一个有趣的选择。`huggingface_hub` 库提供了一种简单的方式来调用运行推理的服务。您可以连接到几种不同的服务：
- [推理 API](https://huggingface.co/docs/api-inference/index)：这个服务允许您在 Hugging Face 的基础设施上免费运行加速推理。这个服务是一个快速入门、测试不同模型和原型化 AI 产品的快速方式。
- [推理端点](https://huggingface.co/docs/inference-endpoints/index)：一个可以轻松将模型部署到生产环境的产品。推理由 Hugging Face 在您选择的云提供商的专用、完全托管的基础设施上运行。

这些服务可以使用 [`InferenceClient`] 对象调用。它作为传统的 [`InferenceApi`] 客户端的替代品，添加了对任务的特定支持，并处理了对 [推理 API](https://huggingface.co/docs/api-inference/index) 和 [推理端点](https://huggingface.co/docs/inference-endpoints/index) 的推理。在 [传统推理API客户端](#legacy-inferenceapi-client) 部分了解如何迁移到新的客户端。
